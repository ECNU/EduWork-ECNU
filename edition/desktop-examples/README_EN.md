...
The ECNU example declares `ecnu-max` as text-only with `input: ["text"]`. Image assistance is enabled by default: `ecnu-plus` observes the image and passes textual evidence to `ecnu-max`, while the original attachment stays in the conversation. The complete university configuration updates with each release without another login. Managed gateways, catalogs and limits follow the new release; personal providers, keys, default model choice and history remain separate and preserved. `features.visionFallback: false` disables assistance for configured enterprise text-only models; native image input on models such as `ecnu-plus` remains available.
