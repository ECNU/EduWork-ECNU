...
RPC methods `ecnuAccountResources.configuration()` and `quota(profileID)` return allowlisted projections only. The Host calls `oidcAccounts.modelResourceFetch(profileID, '/quota', {signal})`, constrained to the configured provider base, GET, managed model Key, no redirects, timeout and logout generation. Identity tokens never go to the model service, and no Key enters client RPC. Account/credential events, disposal and component lifecycles prevent stale account data from appearing. There is no cross-account cache.

The menu preserves actual window balances, progress, reset time, and resource-pack remaining/used/total/expiry/status values. No percentage is invented without a valid total; packs are not blindly added to windows. Detail links allow only credential-free HTTPS URLs. Refresh failure affects quota display only, without deleting login/Keys or blocking models.

See `quota.openapi.yaml` for the existing wire contract. These are ECNU protocol fields, not OIDC standards or mandatory public resource capabilities. Generic OIDC still owns identity, Key and model catalogs. Upgrades do not migrate or overwrite saved identities, personal Keys or user model choices.
