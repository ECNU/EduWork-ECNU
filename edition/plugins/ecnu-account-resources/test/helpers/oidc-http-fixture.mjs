...
export async function fixture(t, options = {}) {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname === '/.well-known/openid-configuration') return json({ issuer: origin,
      ...
      userinfo_endpoint: `${origin}/userinfo`, jwks_uri: `${origin}/jwks`, revocation_endpoint: `${origin}/revoke`,
      response_types_supported: ['code'], grant_types_supported: ['authorization_code','refresh_token'],
      token_endpoint_auth_methods_supported: ['none'], revocation_endpoint_auth_methods_supported: ['none'],
      scopes_supported: ['openid','profile','offline_access','llm:models:read','llm:invoke','llm:profile'],
      id_token_signing_alg_values_supported: ['RS256'], subject_types_supported: ['public'],
      oidc_llm: {version:'0.1',resource:origin,api_base:`${origin}/v1`,identity_modes_supported:['oauth','oidc'],client_registration_methods_supported:['static']},
    ...
    if (url.pathname === '/token') {
      ...
      return json({ access_token: '<ACCESS_TOKEN>', token_type: 'Bearer', expires_in: 3600, refresh_token:'synthetic-refresh', scope:authorize.scope, id_token: `${header}.${payload}.${signature}` })
    }
  ...
  const rawProfile = { schemaVersion: 'dsh-oidc/v1alpha1', id: 'desktop-test', displayName: 'Example organization', brand: options.brand ?? {}, allowInsecureDevelopment: true,
    ...
    ...(options.resources ? {provider:{id:'fixture-ai',adapter:'openai-compatible',modelSource:'discovery'}} : {}),
  }
